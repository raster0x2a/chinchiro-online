...
export function settle(r:Room){const dealer=r.players[r.dealer],dr=judge(dealer.dice);r.log=[];for(const p of r.players){if(p===dealer)continue;if(p.bet===0){p.delta=0;r.log.push(`${p.name}：見学`);continue;}const pr=judge(p.dice);const sign=Math.sign(pr.rank-dr.rank);const winner=sign>0?pr:dr,loser=sign>0?dr:pr;const multiplier=winner.multiple*(loser.rank===-1?2:1);p.delta=sign*p.bet*multiplier;p.chips+=p.delta;dealer.delta-=p.delta;r.log.push(`${p.name}：${p.delta>0?'+':''}${p.delta.toLocaleString()} チップ`)}dealer.chips+=dealer.delta;r.cycleRemaining=remainingDealers(r).filter(id=>id!==dealer.id);r.phase='result';if(r.players.some(p=>p.chips<=0))enterSettlement(r,'negative')}
